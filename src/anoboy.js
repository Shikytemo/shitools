...
const ANOBOY_BASE_URL = 'https://anoboy.cv'
...
const baseUrl = options => String(options.baseUrl || process.env.ANOBOY_BASE_URL || ANOBOY_BASE_URL).replace(/\/+$/, '')
...
const isAnoboyUrl = input => {
	try {
		const url = /^https?:\/\//i.test(input) ? input : `https://${input}`
		return new URL(url).hostname.includes('anoboy')
	} catch {
		return false
	}
}

const isEpisodeUrl = url => /-episode-\d+/i.test(url) || /\/\d+\/?$/i.test(url)
const isSeriesUrl = url => /\/anime\/[^/]+\/?$/i.test(url)

export const getAnoboyEpisodeNumber = text => {
...
export const fetchAnoboyHtml = async (url, options = {}) => {
	...
	try {
		...
		if (!response.ok) {
			throw new Error(`Anoboy request failed ${response.status} ${response.statusText}`)
		}
...
export const parseAnoboyEpisodePage = (html, pageUrl, options = {}) => {
	...
	const seriesUrl =
		...
		absoluteUrl($('.all-ep a[href]').first().attr('href'), options) ||
...
export const parseAnoboySearchPage = (html, options = {}) => {
	const $ = cheerio.load(html)
	const results = []
...
export const parseLatestAnoboyPage = (html, options = {}) => {
	const $ = cheerio.load(html)
	const results = []
	...
	$('.bsx a[href], .listupd .bs a[href]').each((_, element) => {
		...
		results.push({
			...
			url,
			image,
			episode: getAnoboyEpisodeNumber(title) || getAnoboyEpisodeNumber(url),
			type: 'episode'
		})
	})
...
export const parseAnoboySeriesPage = (html, options = {}) => {
	const $ = cheerio.load(html)
	const episodes = []
...
export const getAnoboySeriesEpisodes = async (seriesUrl, options = {}) => {
	if (!seriesUrl) return []

	const html = await fetchAnoboyHtml(seriesUrl, options)
	return parseAnoboySeriesPage(html, options)
}

export const resolveAnoboyEpisode = async ({ seriesUrl, episodeNumber, ...options }) => {
	const episodes = await getAnoboySeriesEpisodes(seriesUrl, options)
...
export const searchAnoboy = async (query, options = {}) => {
	const cleanQuery = cleanText(query)
	if (!cleanQuery) throw new Error('Anoboy query is required')

	const url = `${baseUrl(options)}/?s=${encodeURIComponent(cleanQuery)}`
	const html = await fetchAnoboyHtml(url, options)
	return {
		query: cleanQuery,
		url,
		results: parseAnoboySearchPage(html, options)
	}
}

export const getLatestAnoboy = async (options = {}) => {
	const url = options.url || baseUrl(options)
	const html = await fetchAnoboyHtml(url, options)

	return {
		url,
		results: parseLatestAnoboyPage(html, options)
	}
}

export const getAnoboyStream = async (input, options = {}) => {
	const text = cleanText(input)
	if (!text) throw new Error('Anoboy episode URL or query is required')

	let episodeUrl = ''
	let search = null
	let episodes = []
	const episodeNumber = getAnoboyEpisodeNumber(text)

	if (isAnoboyUrl(text)) {
		const url = absoluteUrl(text, options)
		if (isSeriesUrl(url)) {
			const resolved = await resolveAnoboyEpisode({ ...options, seriesUrl: url, episodeNumber })
		...
		search = await searchAnoboy(seriesSearchQuery(text) || text, options)
		...
		if (directEpisode) {
			...
			const resolved = await resolveAnoboyEpisode({ ...options, seriesUrl: series.url, episodeNumber })
			episodeUrl = resolved.episodeUrl
			episodes = resolved.episodes
		}
	}
	...
	if (!episodeUrl) {
		return {
			ok: false,
			text: 'Anoboy episode not found'
		}
	}

	const html = await fetchAnoboyHtml(episodeUrl, options)
	const episode = parseAnoboyEpisodePage(html, episodeUrl, options)
	if (!episodes.length && episode.seriesUrl) {
		episodes = await getAnoboySeriesEpisodes(episode.seriesUrl, options).catch(() => [])
	}
	...
	if (!episode.mirrors.length) {
		return {
			ok: false,
			text: 'Anoboy stream not found on episode page'
		}
	}
...
export const anoboy = getAnoboyStream